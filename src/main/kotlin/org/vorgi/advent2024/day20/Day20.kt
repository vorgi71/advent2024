...
class Day20 {
  fun start() {
    ...
    val result1 = findCheats(input1, 2, 2).sortedBy { it  }

    println("result1 = ${result1}")

    val result1Map = mutableMapOf<Int, Int>()
    for (value in result1) {
      val count = result1Map.getOrDefault(value, 0) + 1
      result1Map[value] = count
    }

    println("result1Map = ${result1Map}")
    ...
    val result3 = findCheats(input1, 50, 21)
  ...
  private fun findPath(
    startPoint: DistPoint,
    maze: CharGrid,
    visited: List<DistPoint> = listOf(startPoint)
  ): List<List<DistPoint>> {
